...
layui.use(["element", "layer", "okUtils", "okTab", "okLayer", "jQContextMenu"], function () {
  ...
  //加载左边头像
  $("#userimg").attr('src', "images/User/" + $("#EmpyNo").val() + ".png?"+Math.round(Math.random()*10));
  $("#userimg").error(function() {
    $(this).attr("src", "images/User/Error.png?"+Math.round(Math.random()*10));
  })
  //加载右边头像
  $("#ruserimg").attr('src', "images/User/" + $("#EmpyNo").val() + ".png?"+Math.round(Math.random()*10));
  $("#ruserimg").error(function() {
    $(this).attr("src", "images/User/Error.png?"+Math.round(Math.random()*10));
  })
  ...
  //url:"/Vent_war_exploded/userauthority",
url:"data/navs.json",
  //  url:"data/api.json",
    // 允许同时选项卡的个数
    openTabNum: 30,
  ...
  function getContent() {
    ...
    if (dateStr == "2018-12-30" || dateStr == "2018-12-31" || dateStr == "2019-01-01") {
      ...
      content = "龙跃甲子，鸽翱晴空，凤舞九天。<br/>" +
        ...
        "泰山脊，领风骚环宇，有谁堪比？<br/>" +
        "达宏电子 祝您国庆节快乐！<br/>祝福伟大的祖国，越来越强大。<br/>祝福强大的祖国，一直屹立于世界东方！(^し^)";
    } else {
      content = "达宏电子 v3.0上线啦(^し^)<br/>"
    }
    return content;
  }
  ...
  $("#logout").click(function () {
    okLayer.confirm("确定要退出吗？", function (index) {
      window.location = "/Vent_war_exploded/login";
    });
  });
