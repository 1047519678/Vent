...
<script>
    layui.use(["element", "table", "form", "laydate", "okLayer", "okUtils", "okMock"], function () {
        ...
        form.on("submit(search)", function (data) {
            userTable.reload({
                url: "/Phihong/aapinotwechatstate",
                where: data.field,
                page: {curr: 1}
            });
            return false;
        ...
        table.on("tool(tableFilter)", function (obj) {
            ...
            switch (obj.event) {
                case "off":
                    var json = {"EmpyNo": obj.data.EmpyNo, "table": "notwechatstate", "state": "1"}
                    ...
                    break;
                case "on":
                    var json = {"EmpyNo": obj.data.EmpyNo, "table": "notwechatstate", "state": "0"}
                    okUtils.ajax("/update_pushstate", "post", json, true).done(function (response) {
                        okLayer.greenTickMsg(response.msg, function () {
                        })
                    }).fail(function (error) {
